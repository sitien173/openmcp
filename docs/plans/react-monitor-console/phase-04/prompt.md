...
Preserve abort identity in `web/src/lib/api.ts`. When a request signal is
aborted, rethrow the original abort rejection unchanged. Do not wrap it in
`ApiError`. Preserve existing `ApiError` behavior for HTTP, transport, and JSON
failures.
...
Classify aggregate states from underlying query data, not `jobs.length`.
`data !== undefined` means cached usable data, including `[]`. A project-job
error with data is a cached refetch failure. An error without data is an
unavailable project result. Prefer explicit classification fields from
`useAllJobs` over duplicating this logic in `Jobs`. Retain existing return
fields for Phase 3 compatibility.

If every requested project-job query fails without data, show only
`Failed to load jobs.` If at least one project-job query has data while another
fails without data, retain available rows and show exactly
`Could not load jobs for all projects. Showing partial results.` If available
results are empty, show `No jobs found in available results.` A partial warning
takes precedence over the generic refresh warning. Never render an empty state
while any required first-load fan-out remains unresolved. A cached empty
fan-out that later fails remains known-empty and uses the generic refresh
warning plus `No jobs found.`
...
`StatusBadge`. Project displays its alias from the existing
`projectsQuery.data`, falling back to the full `project_id`. Do not add a second
projects observer. Expose the full job ID. Selecting any visible row sets
`selected`.

Keep a native `<button type="button">` in every Jobs row with accessible name
`Open job <full-id>`. Do not turn `<tr>` into a faux button or add a role or tab
stop to it. Optional `DataTable` row activation may handle mouse clicks on
non-interactive cell content, but must ignore interactive descendants so the
native button invokes selection exactly once. Without row activation, rows
remain inert. Retain semantic `<table>`, caption, scoped headers, stable keys,
focusable overflow, and inert-row behavior.
...
Use `padding: var(--space-lg)`, `var(--radius-xs)`,
`var(--motion-normal) var(--ease-standard)`, and existing surface/border
tokens. `--elev-depth4` is the only inspector elevation. Disable nonessential
inspector transitions under `prefers-reduced-motion: reduce`. On desktop, the
list uses `min-width: 0` and the aside is a non-overlay sibling. At the existing
768px breakpoint, switch to one column and make the aside full-width with
border-box sizing. Never use fixed or absolute viewport positioning.
...
Use `JSON.stringify(data, null, 2)`. Render every response exactly in received
array order. Do not sort, reverse, append, or locally merge full-history
responses. Preserve API order. Render `Loading events...`, `Failed to load events.`,
`No events recorded.`, and cached-refetch warning states consistently.

Mount the Inspector only when `selected` is non-empty. Detail and event queries
start independently and in parallel. Keep the timeline mounted and render its
own state while detail loads or fails. Do not gate events on successful detail.
Render cached data whenever `data !== undefined`; initial loading or error
requires absent data. Do not evict caches on close. Do not use previous-ID
placeholder data, aggregate-row detail fallbacks, local result copies, or local
request guards.

On selection change, both observers move to the new ID-bearing keys. On close,
both observers unmount. Switching or closing must abort observable in-flight
detail and event signals, stop future intervals, and never expose cancellation
as a user-facing error. A signal-ignoring late response may populate only its
own ID cache. It must never alter another selection or reopen a closed
inspector. Aggregate Jobs polling remains active after close.

Opening and switching selections use push navigation. Closing uses
`{ replace: true }`. Clone the current search parameters, change or delete only
`selected`, and preserve duplicate unrelated parameters. Missing or empty
`selected` means closed. Browser Back after opening returns to the unselected
URL. Browser Back after A-to-B returns to A.

The Inspector heading is an `h2`; Event Timeline is subordinate. Its aside uses
`aria-labelledby` referencing that heading. Loading uses polite status and
initial errors use alerts. Closing returns focus to the selected row's Open job
button when present, otherwise to the state filter.
...
- Opening and switching use history pushes; Back restores prior selection.
- Duplicate unrelated search parameters survive selection changes.
- Inspector detail fields, status, commit fallbacks, loading, initial error,
  and cached-refetch behavior.
- Detail and timeline render independently and begin in parallel.
...
- Switching and closing abort both observable request signals.
- Aborted API requests preserve their original abort rejection.
- All-failed, partial-with-rows, partial-empty, cached-empty, and unresolved
  fan-out aggregate cases have distinct coverage.
- Project aliases display with project ID fallback.
- Close restores focus to the row button or filter.
- Inspector styling tests cover tokenized motion, reduced motion, desktop
  docking, and narrow stacking.
...
- Detail and event lifecycles remain independent.
- Abort cancellation remains distinguishable from failures.
