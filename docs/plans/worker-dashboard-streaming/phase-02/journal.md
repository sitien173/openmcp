...
- `uv run pytest tests/test_streaming_backends.py tests/test_execution.py tests/test_live_backends.py -m 'not live'`: 48 passed, 3 deselected in 11.36s.
...
| Action | Path | Change |
| --- | --- | --- |
| Modified | src/openmcp/backends/agy.py | Added safe structured-stream parsing and final-only fallback. |
| Modified | src/openmcp/backends/claude.py | Added safe structured-stream parsing and final-only fallback. |
| Modified | src/openmcp/backends/codex.py | Normalized safe JSONL assistant and tool lifecycle events. |
| Modified | src/openmcp/backends/pi.py | Normalized safe JSON assistant and tool lifecycle events. |
| Modified | src/openmcp/drivers.py | Added bounded bridge and cached structured-mode capability checks. |
...
## NOTES
- docs/plans/worker-dashboard-streaming/phase-02/notes.md, Tasks 1 through 4
## SPEC COMPLIANCE
